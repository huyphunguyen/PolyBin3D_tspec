...
class IntegrationUtils:
    ...
    def _tree_flatten(self):
        children = (self.modk, self.degk, self.muk)
        aux_data = {'real_fft': self.real_fft, 'nthreads': self.nthreads}
        return (children, aux_data)

    @classmethod
    def _tree_unflatten(cls, aux_data, children):
        modk, degk, muk = children
        obj = cls.__new__(cls)
        obj.modk = modk
        obj.real_fft = aux_data['real_fft']
        obj.nthreads = aux_data['nthreads']
        obj.degk = degk
        obj.muk = muk
        return obj
class MapUtils:
    ...
    def _tree_flatten(self):
        children = (self.modk, self.muk)
        aux_data = {'gridsize': tuple(int(x) for x in self.gridsize), 'nthreads': self.nthreads}
        return (children, aux_data)
    ...
    def _tree_unflatten(cls, aux_data, children):
        ...
        obj.modk = modk
        obj.muk = muk
        obj.gridsize = aux_data['gridsize']
        obj.nthreads = aux_data['nthreads']
        return obj
